...
from storage.enum import FileType
...
class Storage(ABC):
    ...
    def get_reader_callable(self, fpath: Path) -> Callable:
        filetype = FileType(fpath.suffix)
        if filetype == FileType.CSV:
            return self.get_csv_reader()

    def get_writer_callable(self, fpath: Path) -> Callable:
        filetype = FileType(fpath.suffix)
        if filetype == FileType.CSV:
